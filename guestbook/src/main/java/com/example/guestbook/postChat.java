      ...
      throws IOException {
		...
		try {
			...
			if (chat == null)
			{
			    ...
			    chat.chatContentList = new ArrayList<String>();
			    chat.chatAuthorList = new ArrayList<String>();
			...
			//notes.setProperty("youtubeID", id);
				...
				//for (int i = 0; i < arr.length(); i++) {
					...
					//out.println(comment.getString("content"));
				chat.chatContentList.add(commentObj.getString("content"));		
				chat.chatAuthorList.add(commentObj.getString("user"));		
			}				
			//}
			//if (arr.length() == 0)
			//{
