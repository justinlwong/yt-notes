...
$( document ).ready(function() {
	...
	function updateContent() {
		if (($("#sessionCheck").attr("sessionId") != -1))
		{
			testVar += 1;
		...
		if (($("#chatSessionCheck").attr("chatSessionId") != -1))
		{
			...
			$.get('/getCommentSessions', {user : $("#userCheck").attr("userNick")})
			 .done(function(data) {
				...
				for (var i=0; i< data.sessionList.length; i++)
				{
					...
					$("#menu-container #menu-3 .row").append("<div class='col-md-4 col-ms-6'><div class='g-item'><img src='"+thumbLink+"' alt=''><a data-rel='lightbox' class='overlay' href='"+homeLink+"'><span>"+data.sessionList[i].date+"</span></a></div> <!-- /.g-item --></div> <!-- /.col-md-4 -->");
			    }
	...
	*/
	...
	$("#chatSubmit").click(function() {
		...
		$("#chatContainer").append("<div class='messageDiv' author='"+user+"' message='"+message.replace(/'/g, "&quot")+"'>"+user + " : "+message+"</div>");
		$("#chatBox").val("");
		$("#chatContainer").scrollTop($("#chatContainer").prop('scrollHeight'));
		// 
		//var chatArray = [];
		...
		chatArrayObj.youtubeID = $("#chatYoutubeLink").val();
		chatArrayObj.chatSessionId = $("#chatSessionCheck").attr("chatSessionId");
		chatArrayObj.contentList = chatObj; //chatArray;
